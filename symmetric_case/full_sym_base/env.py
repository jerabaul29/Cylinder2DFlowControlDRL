...
def resume_env(plot=False,
    ...
    simulation_duration = 10.0 #duree en secondes de la simulation
    dt=0.0001
    ...
    geometry_params = {'output': '.'.join([root, 'geo']),
                    ...
                    'jet_width': 10,
                    'clscale': 0.75,
                    'template': '../sym_geometry_2d.template_geo',
                    'remesh': remesh}
    ...
    def profile(mesh, degree):
        ...
        print bot, top
        H = top - bot  # This is half channel

        Um = 1.5

        return Expression(('-4*Um*(x[1]-bot)*(x[1]-top)/H/H',
                           # -top for symmetry
                        '0'), bot=-top, top=top, H=2*H, Um=Um, degree=degree)
    ...
    if geometry_params["jet_positions"] and (geometry_params["jet_positions"][0] - 90) != 0:
